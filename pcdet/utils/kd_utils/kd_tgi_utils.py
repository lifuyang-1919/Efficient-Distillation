...
def _narrow_weight(k, curr_v, self_model_state, spconv_keys):
    ...
    if k in spconv_keys and self_model_state[k].shape[0:3] != curr_v.shape[0:3]:
        ...
        # ===================== 0328 ======================================================
        self_model_state[k] = self_model_state[k].permute(4, 0, 1, 2, 3)
        # ======================1022======================================================
        assert len(self_model_state[k]) <= len(curr_v)
        import random
        t_ran = random.randint(0, len(curr_v) - len(self_model_state[k]))
        # t_ran = torch.randint(0, len(curr_v) - len(self_model_state[k]))
        curr_v = curr_v.narrow(0, t_ran, self_model_state[k].shape[0])
        # =======================curr_v1022===============================================
        for d in range(1, len(self_model_state[k].shape)):  # d:1--唯独
            # import pdb; pdb.set_trace()
            curr_v = curr_v.narrow(d, 0, self_model_state[k].shape[d])
        # =======================curr_v1022===============================================
        curr_v = curr_v.permute(1, 2, 3, 4, 0)
        self_model_state[k] = self_model_state[k].permute(1, 2, 3, 4, 0)
    else:
        for d in range(len(self_model_state[k].shape)):  # d:1--唯独
            # import pdb; pdb.set_trace()
            curr_v = curr_v.narrow(d, 0, self_model_state[k].shape[d])
    return curr_v
