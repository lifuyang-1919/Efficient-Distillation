...
class KDHeadTemplate(object):
    ...
    def parse_teacher_pred_to_targets(self, kd_cfg, pred_boxes_tea, gt_boxes):
        ...
        # final_boxes = torch.zeros((batch_size, max_obj, 10), dtype=torch.float32).cuda()
        final_boxes = torch.zeros((batch_size, max_obj, target_boxes.shape[-1]), dtype=torch.float32).cuda()
