...
class CenterFeatureKDHead(KDHeadTemplate):
    ...
    def get_feature_kd_loss_raw(self, batch_dict, loss_cfg):
        ...
        kd_feature_loss = (kd_feature_loss_all * feature_mask.unsqueeze(1)).sum() / (
                    feature_mask.sum() * channel + 1e-6)
    ...
    def get_feature_kd_loss_rois(self, batch_dict, loss_cfg):
        ...
        if self.model_cfg.FEATURE_KD.ROI_POOL.ROI == 'gt':  # 如果使用GT框作为ROI
            rois = batch_dict['gt_boxes'].detach()
        if self.model_cfg.FEATURE_KD.ROI_POOL.ROIT in ('tea', 'sft', 'soft'):
            if self.model_cfg.FEATURE_KD.ROI_POOL.ROI in ('sft','soft'):
                rois = []
            weis = []
            for b_idx in range(bs):  # bs : batch size
        ...
        if self.model_cfg.FEATURE_KD.ROI_POOL.ROIS == in ('stu', 'sft', 'soft'):
            if self.model_cfg.FEATURE_KD.ROI_POOL.ROI in ('sft'):
                rois = []
                weis = []
            pred_dict_stu = self.dense_head.forward_ret_dict['decoded_pred_dicts']
            rois += [pred_dict_stu[i]['pred_boxes'] for i in range(bs)]
            weis += [pred_dict_stu[i]['pred_scores'] for i in range(bs)]
        else:
            raise NotImplementedError
